...
const SortableFavourite = ({ id, onDelete }: SortableFavouriteProps) => {
	...
	return (
		...
		>
			<AnimatedPaper
				className="favourite-entry"
				color={entry.personal ? "#00FFFF" : undefined}
				border
			>
				<div
				...
				>
					<div
						className="favourite-entry-navigation-button"
						ref={setActivatorNodeRef}
						{...listeners}
					>
						<DragIndicatorIcon
							htmlColor={entry.personal ? "#00FFFF" : "var(--table-border)"}
						/>
					</div>
				</div>
				<div className="favourite-entry-content">
					...
					<div className="favourite-entry-content-button-container">
						...
						<CustomButton
							onClick={handleOpen}
							className="favourite-entry-content-button"
							size="small"
						>
							<OpenInNewIcon />
						</CustomButton>
...
export default memo(SortableFavourite);
