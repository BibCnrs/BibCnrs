...
const DatabaseDisplay = ({
	entry,
	language,
}: { entry: DatabaseEntryDataType; language: string }) => {
	return (
		<li>
			<Tooltip title={getText(entry, language)} arrow>
				...
				>
					<AnimatedPaper className="database-entry-content">
						<div
							className="database-entry-content-inner"
							style={getImage(entry)}
						>
							<span className="database-entry-text">
								{getName(entry, language)}
							</span>
						</div>
					</AnimatedPaper>
				</a>
			...
			<div className="database-icon">
				<DatabaseIcons
					title={getName(entry, language)}
					url={getUrl(entry, language)}
					oa={entry.oa}
				/>
			</div>
		</li>
