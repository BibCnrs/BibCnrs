...
test("Open FAQ item when clicking on toggle", async ({ page }) => {
	await page.goto("/");
	await page.getByRole("link", { name: "FAQ" }).click();
	...
	await page.getByLabel("Déplier En savoir plus sur").click();

	await expect(page.getByText(FULL_TEXT)).toBeVisible();

	await page.getByLabel("Replier En savoir plus sur").click();
